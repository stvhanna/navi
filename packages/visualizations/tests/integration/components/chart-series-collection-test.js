...
test('Component renders list of selected series', function(assert) {
  ...
  assert.ok(!!this.$('.chart-series-collection'),
    'Chart Series Collection component renders');

  let seriesHeaders = this.$('.series-header').map(function() { return this.textContent.trim(); }).get();
  assert.deepEqual(seriesHeaders,
    ['Series 1', 'Series 2'],
  ...
  assert.deepEqual(seriesDimensions,
    expectedDims,
    'Collection contains series with appropriate dimension types');
...
test('Deleting series', function(assert) {
  ...
  this.set('onRemoveSeries', series => {
    ...
    assert.deepEqual(series,
      SELECTED_SERIES_DATA[0],
      'appropriate series data is sent to onRemoveSeries action');
  });
