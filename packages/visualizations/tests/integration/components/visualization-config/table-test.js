...
test('table config - feature flag set', function(assert) {
  ...
  assert.equal(this.$('.table-config__header').text().trim(),
    'Totals',
  ...
  assert.equal(this.$('.table-config__total-toggle-button.x-toggle-component').length,
    2,
...
test('table config - subtotal flag option set', function(assert) {
  ...
  assert.equal(this.$('.table-config__subtotal-dimension-select').text().replace(/\s+/g, " ").trim(),
    'by Operating System',
