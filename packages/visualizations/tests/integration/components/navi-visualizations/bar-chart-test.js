...
module('Integration | Component | bar chart', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.ok(this.$('.navi-vis-c3-chart').is(':visible'),
      'The bar chart widget component is visible');

    assert.equal(findAll('.c3-bar').length,
      5,
      '5 bars are present on the chart');
  });
  ...
  test('multiple metric series', async function(assert) {
    ...
    assert.equal(findAll('.c3-bar').length,
      10,
      'Ten bars are present in the bar based on the metrics in the request');
  });
  ...
  test('multiple dimension series', async function(assert) {
    ...
    await render(TEMPLATE);
    assert.equal(findAll('.c3-bars').length,
      5,
