...
module('Integration | Component | goal gauge ', function(hooks) {
  ...
  test('goal-gauge renders correctly', async function(assert) {
    ...
    assert.equal(find('.value-title').textContent,
      '3.03B',
      'value title is correctly displayed');

    assert.equal(find('.metric-title').textContent,
      'Page Views',
      'the default metric title is correctly displayed');

    assert.equal(find('.c3-chart-arcs-gauge-min').textContent,
      '290M',
      'min gauge label is correctly displayed');

    assert.equal(find('.c3-chart-arcs-gauge-max').textContent,
      '310M',
      'max gauge label is correctly displayed');

    assert.equal(find('.goal-title').textContent.trim(),
      '310M Goal',
  ...
  test('goal-gauge title class is based on actualValue vs baselineValue', async function(assert) {
    ...
    `);
    assert.ok(!!findAll('.value-title.pos').length,
    ...
    `);
    assert.ok(!!findAll('.value-title.neg').length,
  ...
  test('cleanup', async function(assert) {
    ...
    assert.equal(findAll('text.c3-chart-arcs-title > tspan').length,
      3,
