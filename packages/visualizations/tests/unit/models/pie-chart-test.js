...
module('Unit | Model | Pie Chart Visualization Fragment', function(hooks) {
  ...
  test('default value', function(assert) {
    ...
    let metricsAndDims = [
        ...
        chart = run(() => run(() => this.owner.lookup('service:store').createRecord('all-the-fragments')).get('pieChart'));
  ...
  test('chart type', function(assert) {
    ...
    let chart = run(() => run(() => this.owner.lookup('service:store').createRecord('all-the-fragments')).get('pieChart'));
