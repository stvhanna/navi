...
module('Unit | Model | Line Chart Visualization Fragment', function(hooks) {
  ...
  test('default value', function(assert) {
    ...
    let metricsAndDims = [ [{metric: 'm1', parameters: {}}, {metric: 'm2', parameters: {}}], ['d1', 'd2'] ],
        chart = run(() => run(() => this.owner.lookup('service:store').createRecord('all-the-fragments')).get('lineChart'));
  ...
  test('chart type', function(assert) {
    ...
    let chart = run(() => run(() => this.owner.lookup('service:store').createRecord('all-the-fragments')).get('lineChart'));
