...
module('Unit | Model | Metric Label Visualization Fragment', function(hooks) {
  ...
  test('isValidForRequest', function(assert) {
    ...
    let request = buildTestRequest([{metric: 'rupees', parameters: {}}], []),
        metricLabel = run(() => run(() => this.owner.lookup('service:store').createRecord('all-the-fragments')).get('metricLabel'));
  ...
  test('rebuildConfig', function(assert) {
    ...
    let metricLabel = run(() => run(() => this.owner.lookup('service:store').createRecord('all-the-fragments')).get('metricLabel'));
