...
module('Unit | Component | table', function(hooks) {
  ...
  hooks.beforeEach(function() {
    this.owner.register('component:navi-table-sort-icon', Component.extend(), {instantiate: false});
    this.owner.register('component:sortable-group', Component.extend(), {instantiate: false});
    this.owner.register('component:sortable-item', Component.extend(), {instantiate: false});
    this.owner.register('component:navi-icon', Component.extend(), {instantiate: false});
    this.owner.register('component:tooltip-on-element', Component.extend(), {instantiate: false});

    //helpers
    this.owner.register('helper:and', buildHelper(()=>{}), {instantiate: false});
    this.owner.register('helper:sub', buildHelper(()=>{}), {instantiate: false});
    this.owner.register('helper:not-eq', buildHelper(()=>{}), {instantiate: false});
    this.owner.register('helper:is-valid-moment', buildHelper(()=>{}), {instantiate: false});
    this.owner.register('helper:format-number', buildHelper(()=>{}), {instantiate: false});
  ...
  test('computeTotal and computeSubtotals', function(assert) {
    ...
    let options = $.extend(true, {}, OPTIONS, { showTotals: { subtotal: 'dimension' }}),
        component = this.owner.factoryFor('component:navi-visualizations/table').create({ options, model: A([{ response: { rows: ROWS } }]) });
