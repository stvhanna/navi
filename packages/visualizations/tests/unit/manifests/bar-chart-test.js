...
test('bar chart visualization type is valid', function(assert) {
  ...
  assert.notOk(manifest.typeIsValid(request),
    'bar chart type is invalid for single time bucket no dimension');

  // valid for single time bucket with dimension
  set(request, 'dimensions', ['sword']);
  assert.ok(manifest.typeIsValid(request),
    'bar chart type is valid for single time bucket with dimension');
  ...
  assert.ok(manifest.typeIsValid(request),
    'bar chart type is valid for multiple time buckets with dimension');

  // valid for multiple time bucket no dimension
  set(request, 'dimensions', []);
  assert.ok(manifest.typeIsValid(request),
    'bar chart type is invalid for multiple time bucket no dimension');
});
