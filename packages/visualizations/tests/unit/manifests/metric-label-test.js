...
module('Unit | Manifests | metric label', function(hooks) {
  ...
  test('metric label visualization type is valid', function(assert) {
    ...
    assert.deepEqual(manifest.typeIsValid(request),
      true,
    ...
    assert.deepEqual(manifest.typeIsValid(request),
      false,
      'metric label is invalid due to multiple rows');

    // multiple metrics
    set(request, 'metrics', [{ metric: 'adClicks' },{ metric: 'navClicks' }]);
    assert.deepEqual(manifest.typeIsValid(request),
      false,
      'metric label is invalid due to multiple metrics');
    ...
    assert.deepEqual(manifest.typeIsValid(request),
      false,
      'metric label is invalid due to multiple time buckets');
  });
});
